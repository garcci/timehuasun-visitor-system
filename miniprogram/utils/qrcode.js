/**
 * QR Code generator for WeChat Miniprogram
 * Adapted from qrcode-generator by Kazuhiko Arase (MIT License)
 * and weapp-qrcode by tomfriwel (MIT License)
 */
  ...
  function getRSBlocks(typeNumber, errorCorrectLevel) {
    ...
    for (var i = 0; i < rsBlock.length; i += 3) {
      for (var j = 0; j < rsBlock[i]; j++) {
        list.push({ totalCount: rsBlock[i + 1], dataCount: rsBlock[i + 2] });
      }
    }
    return list;
  }
...
QRCodeModel.prototype = {
  ...
  make: function () { this._make(false, this._getBestMaskPattern()); },
  _make: function (test, maskPattern) {
    this.moduleCount = this.typeNumber * 4 + 17;
    this.modules = [];
    for (var i = 0; i < this.moduleCount; i++) {
      this.modules.push([]);
      for (var j = 0; j < this.moduleCount; j++) this.modules[i].push(null);
    }
  ...
  _getBestMaskPattern: function () {
    ...
    for (var i = 0; i < 8; i++) {
      this._make(true, i);
      var lostPoint = QRCodeModel._getLostPoint(this);
  ...
  _setupPositionAdjustPattern: function () {
    var pos = QRCodeModel._getPatternPosition(this.typeNumber);
  ...
  _setupTypeNumber: function (test) {
    var bits = QRCodeModel._getBCHTypeNumber(this.typeNumber);
    for (var i = 0; i < 18; i++) {
  ...
  _setupTypeInfo: function (test, maskPattern) {
    ...
    var bits = QRCodeModel._getBCHTypeInfo(data);
    for (var i = 0; i < 15; i++) {
      ...
      while (true) {
        for (var c = 0; c < 2; c++) {
          if (this.modules[row][col - c] == null) {
            ...
            var mask = QRCodeModel._getMask(maskPattern, row, col - c);
            if (mask) dark = !dark;
...
QRCodeModel._getMask = function (maskPattern, i, j) {
  switch (maskPattern) {
...
QRCodeModel._getPatternPosition = function (typeNumber) {
  return [[], [6, 18], [6, 22], [6, 26], [6, 30], [6, 34], [6, 22, 38], [6, 24, 42], [6, 26, 46],
...
QRCodeModel._getBCHTypeInfo = function (data) {
  var d = data << 10;
  while (QRCodeModel._getBCHDigit(d) - QRCodeModel._getBCHDigit(1335) >= 0)
    d ^= (1335 << (QRCodeModel._getBCHDigit(d) - QRCodeModel._getBCHDigit(1335)));
  return ((data << 10) | d) ^ 21522;
};
QRCodeModel._getBCHTypeNumber = function (data) {
  var d = data << 12;
  while (QRCodeModel._getBCHDigit(d) - QRCodeModel._getBCHDigit(7973) >= 0)
    d ^= (7973 << (QRCodeModel._getBCHDigit(d) - QRCodeModel._getBCHDigit(7973)));
  return (data << 12) | d;
};
QRCodeModel._getBCHDigit = function (data) {
  var digit = 0;
  while (data != 0) { digit++; data >>>= 1; }
  return digit;
};
QRCodeModel._getLostPoint = function (qrCode) {
...
QRCodeModel._createData = function (typeNumber, errorCorrectLevel, dataList) {
  ...
  for (var i = 0; i < dataList.length; i++) {
    ...
    buffer.put(data.getLength(), QRCodeModel._getLengthInBits(data.mode, typeNumber));
    data.write(buffer);
  }
...
QRCodeModel._getLengthInBits = function (mode, type) {
  if (mode == 4) {
    if (type < 10) return 8;
...
QRCodeModel._createBytes = function (buffer, rsBlocks) {
  ...
  for (var r = 0; r < rsBlocks.length; r++) {
    ...
    offset += dcCount;
    var rsPoly = QRCodeModel._getErrorCorrectPolynomial(ecCount);
...
QRCodeModel._getErrorCorrectPolynomial = function (errorCorrectLength) {
  var a = new QRPolynomial([1], 0);
  for (var i = 0; i < errorCorrectLength; i++) a = a.multiply(new QRPolynomial([1, QRMath.gexp(i)], 0));
  return a;
};

function _getTypeNumber(data) {
  var length = data.length;
  for (var typeNumber = 1; typeNumber <= 40; typeNumber++) {
    var rsBlocks = QRRSBlock.getRSBlocks(typeNumber, QRErrorCorrectLevel.M);
    var totalDataCount = rsBlocks.reduce(function (s, b) { return s + b.dataCount; }, 0);
    if (totalDataCount >= length + 3) return typeNumber;
  }
  throw new Error('Too long data');
}

/**
 * Draw QR Code on WeChat miniprogram canvas
 ...
 * @param {number} [options.correctLevel=QRErrorCorrectLevel.M]
 * @param {object} [options.componentContext] - Component 'this' context (required inside Component)
 */
function QRCode(options) {
  ...
  var correctLevel = options.correctLevel !== undefined ? options.correctLevel : QRErrorCorrectLevel.M;
  var canvasId = options.canvasId;
  var componentContext = options.componentContext;
  ...
  var ctx = componentContext
    ? wx.createCanvasContext(canvasId, componentContext)
    : wx.createCanvasContext(canvasId);

  var moduleCount = qr.getModuleCount();
  var cellW = width / moduleCount;
  var cellH = height / moduleCount;
  ...
  for (var row = 0; row < moduleCount; row++) {
    for (var col = 0; col < moduleCount; col++) {
      ...
      var x = col * cellW;
      var y = row * cellH;
      ctx.fillRect(x, y, cellW + 0.1, cellH + 0.1);
    }
  }
  ctx.draw();
}
